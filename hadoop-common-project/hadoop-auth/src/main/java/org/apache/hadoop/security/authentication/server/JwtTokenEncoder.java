...
public class JwtTokenEncoder implements TokenEncoder {
  private static Logger LOG = LoggerFactory
    .getLogger(JwtTokenEncoder.class);
  ...
  @Override
  public byte[] encodeAsBytes(AuthToken token) throws IOException {
    String tokenStr = encodeAsString(token);
    return tokenStr.getBytes();
  }

  @Override
  public String encodeAsString(AuthToken token) throws IOException {
    if (!(token instanceof JwtAuthToken)) {
      throw new IOException("Unexpected AuthToken, not JwtAuthToken");
    }
    ...
    if (signKey != null) {
      ...
      try {
        ...
        LOG.warn("Unable to sign the Signed JWT", e);
      }
      // Encrypt
      if (encryptionKey != null) {
        ...
        try {
          ...
          LOG.warn("Unable to encrypt the JWE object", e);
        }
      ...
      try {
        ...
        LOG.warn("Unable to encrypt the encrypted JWT", e);
      }
